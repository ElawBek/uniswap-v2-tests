...
describe("APP", () => {
  ...
  describe("Add liquidity", () => {
    ...
    it("Add liquidity with pair creation", async () => {
      ...
      const createdPair = await Factory.getPair(
        TokenOne.address,
        TokenTwo.address
      );
      ...
      expect(
        await PairERCtoERC.connect(userOne).balanceOf(userOne.address)
      ).to.be.eq(parseEther("9.999999999999999000"));
    ...
    it("Add liquidity with ETH", async () => {
      ...
      // 1000 - MINIMUM_LIQUIDITY
      expect(
        await PairERCtoWETH.connect(userOne).balanceOf(userOne.address)
      ).to.be.eq(parseEther("44.721359549995792928"));
    ...
    it("Add liquidity with a token that has a fee", async () => {
      ...
      await tokenWithFee
        .connect(userOne)
        .approve(Router.address, parseEther("200"));
      ...
      expect(
        await PairERCtoERC.connect(userOne).balanceOf(userOne.address)
      ).to.be.eq(parseEther("44.654227123532212252"));
    });
    ...
    it("Add liquidity with a token that has a 6 decimals", async () => {
      ...
      // 1000 - MINIMUM_LIQUIDITY
      expect(
        await PairERCtoERC.connect(userTwo).balanceOf(userTwo.address)
      ).to.be.eq(parseEther("0.000063245553202367"));
    });
  });
});
