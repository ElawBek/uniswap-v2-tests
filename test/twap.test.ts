...
describe('APP', () => {
	...
	describe('Flash Swap', () => {
		beforeEach(async () => {
			...
			TWAP = await new UniswapV2TWAP__factory(owner).deploy(PairERCtoERC.address)
		})
		...
		it('Checking the functionality', async () => {
			...
			await TWAP.update()
			console.log('price0CumulativeLast', formatEther(await TWAP.price0CumulativeLast()))
			console.log('price1CumulativeLast', formatEther(await TWAP.price1CumulativeLast()))
			console.log('blockTimestampLast', await TWAP.blockTimestampLast())
			...
			console.log(formatEther(await TWAP.consult(TokenOne.address, parseEther('120'))))
		})
	})
})
