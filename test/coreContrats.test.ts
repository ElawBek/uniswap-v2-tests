...
describe("APP", () => {
  ...
  describe("Factory", () => {
    it("Change feeSetter and feeTo", async () => {
      ...
      await expect(
        Factory.connect(userOne).setFeeToSetter(userOne.address)
      ).to.be.revertedWith("UniswapV2: FORBIDDEN");

      await expect(
        Factory.connect(userOne).setFeeTo(userOne.address)
      ).to.be.revertedWith("UniswapV2: FORBIDDEN");
    ...
    it("Create pair", async () => {
      ...
      expect(
        await Factory.getPair(TokenOne.address, TokenTwo.address)
      ).to.be.eq(constants.AddressZero);

      await expect(
        Factory.createPair(TokenOne.address, TokenOne.address)
      ).to.be.revertedWith("UniswapV2: IDENTICAL_ADDRESSES");

      await expect(
        Factory.createPair(TokenOne.address, constants.AddressZero)
      ...
      await Factory.connect(userOne).createPair(
        WETH.address,
        TokenThree.address
      );
      ...
      const createdPairERCtoERC = await Factory.getPair(
        TokenTwo.address,
        TokenOne.address
      );
      const createdPairERCtoWETH = await Factory.getPair(
        WETH.address,
        TokenThree.address
      );
  ...
  describe("Pair", () => {
    ...
    it("Checking variables", async () => {
      expect(await PairERCtoERC.connect(owner).factory()).to.eq(
        Factory.address
      );
      expect(await PairERCtoERC.connect(owner).token0()).to.eq(
        TokenOne.address
      );
      expect(await PairERCtoERC.connect(owner).token1()).to.eq(
        TokenTwo.address
      );

      const { _reserve0, _reserve1 } = await PairERCtoERC.connect(
        owner
      ).getReserves();
